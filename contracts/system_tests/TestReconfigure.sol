...
contract TestReconfigureProject is TestBaseWorkflow {
  ...
  SPFundingCycleData _dataReconfiguration;
  SPFundingCycleData _dataWithoutBallot;
  SPFundingCycleMetadata _metadata;
  SPReconfigurationBufferBallot _ballot;
  ...
  function setUp() public override {
    ...
    _ballot = new SPReconfigurationBufferBallot(BALLOT_DURATION, SPFundingCycleStore());
    ...
    _dataWithoutBallot = SPFundingCycleData({
      duration: 6 days,
      weight: 1000 * 10**18,
      discountRate: 0,
      ballot: SPReconfigurationBufferBallot(address(0))
    });

    _dataReconfiguration = SPFundingCycleData({
      duration: 6 days,
      weight: 69 * 10**18,
      discountRate: 0,
      ballot: SPReconfigurationBufferBallot(address(0))
    ...
    _terminals = [SPETHPaymentTerminal()];
  }
  ...
  function testReconfigureProject() public {
    ...
    SPFundingCycle memory fundingCycle = SPFundingCycleStore().currentOf(projectId);
    ...
    fundingCycle = SPFundingCycleStore().currentOf(projectId);
    ...
    SPFundingCycle memory newFundingCycle = SPFundingCycleStore().currentOf(projectId);
  ...
  function testMultipleReconfigurationOnRolledOver() public {
    ...
    SPFundingCycle memory queuedFundingCycle = SPFundingCycleStore().queuedOf(projectId);
  ...
  function testMultipleReconfigure(uint8 FUZZED_BALLOT_DURATION) public {
    _ballot = new SPReconfigurationBufferBallot(FUZZED_BALLOT_DURATION, SPFundingCycleStore());
    ...
    SPFundingCycle memory initialFundingCycle = SPFundingCycleStore().currentOf(projectId);
    SPFundingCycle memory currentFundingCycle = initialFundingCycle;
    SPFundingCycle memory queuedFundingCycle = SPFundingCycleStore().queuedOf(projectId);

    evm.warp(currentFundingCycle.start + 1); // Avoid overwriting current fc while reconfiguring

    for (uint256 i = 0; i < 4; i++) {
      currentFundingCycle = SPFundingCycleStore().currentOf(projectId);
      ...
      currentFundingCycle = SPFundingCycleStore().currentOf(projectId);
      queuedFundingCycle = SPFundingCycleStore().queuedOf(projectId);
  ...
  function testReconfigureProjectFuzzRates(
    ...
    SPETHPaymentTerminal().pay{value: BALANCE}(
      projectId,
      BALANCE,
      address(0),
      _beneficiary,
    ...
    if (BALANCE != 0)
      assertEq(SPTokenStore().balanceOf(_beneficiary, projectId), _userTokenBalance);
    ...
    if (BALANCE != 0)
      assertEq(
        SPTokenStore().balanceOf(_beneficiary, projectId),
        _userTokenBalance + _newUserTokenBalance
      );

    uint256 tokenBalance = SPTokenStore().balanceOf(_beneficiary, projectId);
    uint256 totalSupply = SPController().totalOutstandingTokensOf(projectId, RESERVED_RATE);
    uint256 overflow = SPETHPaymentTerminal().currentEthOverflowOf(projectId);

    evm.startPrank(_beneficiary);
    SPETHPaymentTerminal().redeemTokensOf(
      _beneficiary,
      projectId,
      tokenBalance,
  ...
  function testLaunchProjectWrongBallot() public {
    ...
    SPFundingCycleData memory _dataNew = SPFundingCycleData({
      duration: 6 days,
      weight: 12345 * 10**18,
      discountRate: 0,
      ballot: ISPFundingCycleBallot(address(6969)) // Wrong ballot address
    });
