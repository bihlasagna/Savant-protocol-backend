...
contract TestERC20Terminal is TestBaseWorkflow {
  ...
  function setUp() public override {
    ...
    _terminals.push(SPERC20PaymentTerminal());
  }

  function testAllowanceERC20() public {
    SPERC20PaymentTerminal terminal = SPERC20PaymentTerminal();
    ...
    SPToken().approve(address(terminal), 20 * 10**18);
    evm.prank(caller); // back to regular msg.sender (bug?)
    terminal.pay(
      projectId,
      20 * 10**18,
    ...
    assertEq(SPPaymentTerminalStore().balanceOf(terminal, projectId), 20 * 10**18);
    ...
    assertEq(
      ...
      PRBMath.mulDiv(5 * 10**18, SPLibraries().MAX_FEE(), SPLibraries().MAX_FEE() + terminal.fee())
    );

    // Distribute the funding target ETH -> splits[] is empty -> everything in left-over, to project owner
    uint256 initBalance = SPToken().balanceOf(_projectOwner);
    evm.prank(_projectOwner);
    ...
    assertEq(
      SPToken().balanceOf(_projectOwner),
      initBalance +
        (10 * 10**18 * SPLibraries().MAX_FEE()) /
        (terminal.fee() + SPLibraries().MAX_FEE())
    );
    ...
    // verify: beneficiary should have a balance of 0 SPTokens
  ...
  function testFuzzedAllowanceERC20(
    ...
    evm.assume(SPToken().totalSupply() >= BALANCE);
    ...
    evm.label(caller, 'caller');
    evm.prank(_projectOwner);
    SPToken().transfer(caller, BALANCE);

    evm.prank(caller); // back to regular msg.sender (bug?)
    SPToken().approve(address(terminal), BALANCE);
    evm.prank(caller); // back to regular msg.sender (bug?)
    ...
    assertEq(SPPaymentTerminalStore().balanceOf(terminal, projectId), BALANCE);
    ...
    if (BALANCE != 0 && !willRevert)
      assertEq(
        SPToken().balanceOf(msg.sender),
        PRBMath.mulDiv(ALLOWANCE, SPLibraries().MAX_FEE(), SPLibraries().MAX_FEE() + terminal.fee())
      );
    ...
    if (TARGET <= BALANCE && TARGET != 0)
      assertEq(
        SPToken().balanceOf(_projectOwner),
        initBalance +
          PRBMath.mulDiv(TARGET, SPLibraries().MAX_FEE(), terminal.fee() + SPLibraries().MAX_FEE())
      );
