...
contract TestAllowance is TestBaseWorkflow {
  ...
  function setUp() public override {
    ...
    _terminals.push(SPETHPaymentTerminal());
  }
  ...
  function testAllowance() public {
    ...
    assertEq(SPPaymentTerminalStore().balanceOf(terminal, projectId), 20 ether);
    ...
    assertEq(
      ...
      PRBMath.mulDiv(5 ether, SPLibraries().MAX_FEE(), SPLibraries().MAX_FEE() + terminal.fee())
    );
    ...
    assertEq(
      ...
      (10 ether * SPLibraries().MAX_FEE()) / (terminal.fee() + SPLibraries().MAX_FEE())
    );
  ...
  function testFuzzAllowance(
    ...
    uint256 CURRENCY = SPLibraries().ETH(); // Avoid testing revert on this call...
    ...
    if (TARGET <= BALANCE && TARGET != 0)
      assertEq(
        _projectOwner.balance,
        (TARGET * SPLibraries().MAX_FEE()) / (terminal.fee() + SPLibraries().MAX_FEE())
      );
  }
}
