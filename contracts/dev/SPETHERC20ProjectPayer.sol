...
import './interfaces/ISPProjectPayer.sol';
...
/** 
  ...
  @dev
  Adheres to -
  ISPProjectPayer:  General interface for the methods in this contract that interact with the blockchain's state according to the protocol's rules.
...
*/
contract SPETHERC20ProjectPayer is ISPProjectPayer, Ownable, ERC165 {
  //*********************************************************************//
  // -------------------------- custom errors -------------------------- //
  //*********************************************************************//
  ...
  */
  ...
  {
    return
      _interfaceId == type(ISPProjectPayer).interfaceId || super.supportsInterface(_interfaceId);
  }
  ...
  */
  function _pay(
    ...
    ISPPaymentTerminal _terminal = directory.primaryTerminalOf(_projectId, _token);

    // There must be a terminal.
    if (_terminal == ISPPaymentTerminal(address(0))) revert TERMINAL_NOT_FOUND();
    ...
    if (_token != SPTokens.ETH) IERC20(_token).approve(address(_terminal), _amount);

    // If the token is ETH, send it in msg.value.
    uint256 _payableValue = _token == SPTokens.ETH ? _amount : 0;

    // Send funds to the terminal.
    // If the token is ETH, send it in msg.value.
    _terminal.pay{value: _payableValue}(
      _projectId,
      _amount, // ignored if the token is SPTokens.ETH.
      _token,
