...
import './interfaces/ISPSplitsPayer.sol';
import './interfaces/ISPSplitsStore.sol';
...
/** 
  ...
  @dev
  Adheres to -
  ISPSplitsPayer:  General interface for the methods in this contract that interact with the blockchain's state according to the protocol's rules.

  @dev
  Inherits from -
  SPETHERC20ProjectPayer: Sends ETH or ERC20's to a project treasury as it receives direct payments or has it's functions called.
  ReentrancyGuard: Contract module that helps prevent reentrant calls to a function.
*/
contract SPETHERC20SplitsPayer is ISPSplitsPayer, SPETHERC20ProjectPayer, ReentrancyGuard {
  //*********************************************************************//
  ...
  */
  function supportsInterface(bytes4 _interfaceId)
    ...
    view
    virtual
    override(SPETHERC20ProjectPayer, IERC165)
    returns (bool)
  {
    return
      _interfaceId == type(ISPSplitsPayer).interfaceId || super.supportsInterface(_interfaceId);
  }
  ...
  */
  function pay(
    ...
    if (address(_token) != SPTokens.ETH) {
  ...
  */
  function _payToSplits(
    ...
    SPSplit[] memory _splits = splitsStore.splitsOf(_splitsProjectId, _splitsDomain, _splitsGroup);
    ...
    for (uint256 i = 0; i < _splits.length; i++) {
      ...
      SPSplit memory _split = _splits[i];
      ...
      if (_splitAmount > 0) {
        ...
        if (_split.allocator != ISPSplitAllocator(address(0))) {
          ...
          uint256 _payableValue = _token == SPTokens.ETH ? _splitAmount : 0;
