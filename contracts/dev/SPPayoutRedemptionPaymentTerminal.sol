...
import './../interfaces/ISPController.sol';
import './../interfaces/ISPPayoutRedemptionPaymentTerminal.sol';
import './../libraries/SPConstants.sol';
import './../libraries/SPCurrencies.sol';
import './../libraries/SPFixedPointNumber.sol';
import './../libraries/SPFundingCycleMetadataResolver.sol';
import './../libraries/SPOperations.sol';
import './../libraries/SPSplitsGroups.sol';
import './../libraries/SPTokens.sol';
import './../structs/SPTokenAmount.sol';
import './SPOperatable.sol';
import './SPSingleTokenPaymentTerminal.sol';
...
/**
  ...
  @dev
  Adheres to -
  ISPPayoutRedemptionPaymentTerminal: General interface for the methods in this contract that interact with the blockchain's state according to the protocol's rules.

  @dev
  Inherits from -
  SPSingleTokenPaymentTerminal: Generic terminal managing all inflows of funds into the protocol ecosystem for one token.
...
*/
abstract contract SPPayoutRedemptionPaymentTerminal is
  ISPPayoutRedemptionPaymentTerminal,
  SPSingleTokenPaymentTerminal,
  SPOperatable,
  Ownable,
  ReentrancyGuard
{
  // A library that parses the packed funding cycle metadata into a friendlier format.
  using SPFundingCycleMetadataResolver for SPFundingCycle;
  ...
  */
  mapping(uint256 => SPFee[]) internal _heldFeesOf;
  ...
  */
  ISPSplitsStore public immutable override splitsStore;

  /**
    @notice
    The contract that exposes price feeds.
  */
  ISPPrices public immutable override prices;

  /**
    @notice
    The contract that stores and manages the terminal's data.
  */
  ISPSingleTokenPaymentTerminalStore public immutable override store;
  ...
  */
  ISPFeeGauge public override feeGauge;
  ...
  */
    ...
    returns (uint256)
  {
    ...
    uint256 _adjustedOverflow = (decimals == 18)
      ? _overflow
      : SPFixedPointNumber.adjustDecimals(_overflow, decimals, 18);

    // Return the amount converted to ETH.
    return
      (currency == SPCurrencies.ETH)
        ? _adjustedOverflow
        : PRBMath.mulDiv(
          _adjustedOverflow,
          10**decimals,
          prices.priceFor(currency, SPCurrencies.ETH, decimals)
        );
  }
  ...
  */
  function heldFeesOf(uint256 _projectId) external view override returns (SPFee[] memory) {
  ...
  */
  function supportsInterface(bytes4 _interfaceId)
    ...
    view
    virtual
    override(SPSingleTokenPaymentTerminal, IERC165)
    returns (bool)
  {
    return
      _interfaceId == type(ISPPayoutRedemptionPaymentTerminal).interfaceId ||
      _interfaceId == type(ISPPayoutTerminal).interfaceId ||
      _interfaceId == type(ISPAllowanceTerminal).interfaceId ||
      _interfaceId == type(ISPRedemptionTerminal).interfaceId ||
  ...
  */
  constructor(
    ...
    payable
    SPSingleTokenPaymentTerminal(_token, _decimals, _currency)
  ...
  */
  function redeemTokensOf(
    ...
    virtual
    override
    requirePermission(_holder, _projectId, SPOperations.REDEEM)
  ...
  */
  function useAllowanceOf(
    ...
    virtual
    override
    requirePermission(projects.ownerOf(_projectId), _projectId, SPOperations.USE_ALLOWANCE)
  ...
  */
  function migrate(uint256 _projectId, ISPPaymentTerminal _to)
    external
    virtual
    override
    requirePermission(projects.ownerOf(_projectId), _projectId, SPOperations.MIGRATE_TERMINAL)
    returns (uint256 balance)
  {
    ...
    if (balance > 0) {
      ...
      uint256 _payableValue = token == SPTokens.ETH ? balance : 0;
  ...
  */
  function processFees(uint256 _projectId)
    ...
    requirePermissionAllowingOverride(
      ...
      _projectId,
      SPOperations.PROCESS_FEES,
      msg.sender == owner()
    )
  {
    // Get a reference to the project's held fees.
    SPFee[] memory _heldFees = _heldFeesOf[_projectId];
  ...
  */
  function setFeeGauge(ISPFeeGauge _feeGauge) external virtual override onlyOwner {
  ...
  */
  function _redeemTokensOf(
    ...
    // Scoped section prevents stack too deep. `_delegate` only used within scope.
    {
      ...
      if (_tokenCount > 0)
        ISPController(directory.controllerOf(_projectId)).burnTokensOf(
          _holder,
          _projectId,
          _tokenCount,
          '',
      ...
      if (_delegate != ISPRedemptionDelegate(address(0))) {
        SPDidRedeemData memory _data = SPDidRedeemData(
          _holder,
          _projectId,
          _fundingCycle.configuration,
          _tokenCount,
          SPTokenAmount(token, reclaimAmount, decimals, currency),
          _beneficiary,
          _memo,
          _metadata
        );
  ...
  */
  function _distributePayoutsOf(
    ...
    (SPFundingCycle memory _fundingCycle, uint256 _distributedAmount) = store.recordDistributionFor(
      _projectId,
      _amount,
      _currency
    );
    ...
    // Scoped section prevents stack too deep. `_feeDiscount`, `_feeEligibleDistributionAmount`, and `_leftoverDistributionAmount` only used within scope.
    {
      ...
      // Take the fee.
      _fee = _feeDiscount == SPConstants.MAX_FEE_DISCOUNT || _feeEligibleDistributionAmount == 0
        ? 0
        : _takeFeeFrom(
          _projectId,
          _fundingCycle,
  ...
  */
  function _useAllowanceOf(
    ...
    (SPFundingCycle memory _fundingCycle, uint256 _distributedAmount) = store.recordUsedAllowanceOf(
      _projectId,
      _amount,
      _currency
    );
    ...
    // Scoped section prevents stack too deep. `_fee`, `_projectOwner`, `_feeDiscount`, and `_netAmount` only used within scope.
    {
      ...
      _fee = _feeDiscount == SPConstants.MAX_FEE_DISCOUNT
        ? 0
  ...
  */
  function _distributeToPayoutSplitsOf(
    ...
    SPSplit[] memory _splits = splitsStore.splitsOf(_projectId, _domain, _group);

    // Transfer between all splits.
    for (uint256 _i = 0; _i < _splits.length; ) {
      // Get a reference to the split being iterated on.
      SPSplit memory _split = _splits[_i];

      // The amount to send towards the split.
      uint256 _payoutAmount = PRBMath.mulDiv(
        _amount,
        _split.percent,
        SPConstants.SPLITS_TOTAL_PERCENT
      );
      ...
      if (_payoutAmount > 0) {
        ...
        if (_split.allocator != ISPSplitAllocator(address(0))) {
          ...
          if (isFeelessAddress[address(_split.allocator)])
            ...
            unchecked {
              _netPayoutAmount = _feeDiscount == SPConstants.MAX_FEE_DISCOUNT
                ? _payoutAmount
          ...
          // If this terminal's token is ETH, send it in msg.value.
          uint256 _payableValue = token == SPTokens.ETH ? _netPayoutAmount : 0;

          // Create the data to send to the allocator.
          SPSplitAllocationData memory _data = SPSplitAllocationData(
            token,
            _netPayoutAmount,
            decimals,
            _projectId,
          ...
          ISPPaymentTerminal _terminal = directory.primaryTerminalOf(_split.projectId, token);

          // The project must have a terminal to send funds to.
          if (_terminal == ISPPaymentTerminal(address(0))) revert TERMINAL_IN_SPLIT_ZERO_ADDRESS();
  ...
  */
  function _takeFeeFrom(
    ...
    if (_fundingCycle.shouldHoldFees()) {
      // Store the held fee.
      _heldFeesOf[_projectId].push(SPFee(_amount, uint32(fee), uint32(_feeDiscount), _beneficiary));
  ...
  */
  function _processFee(uint256 _amount, address _beneficiary) internal {
    ...
    ISPPaymentTerminal _terminal = directory.primaryTerminalOf(_PROTOCOL_PROJECT_ID, token);
    ...
    if (_terminal == this)
      ...
      // If this terminal's token is ETH, send it in msg.value.
      uint256 _payableValue = token == SPTokens.ETH ? _amount : 0;
  ...
  */
  function _pay(
    ...
    {
      ISPPayDelegate _delegate;
      uint256 _tokenCount;

      // Bundle the amount info into a SPTokenAmount struct.
      SPTokenAmount memory _bundledAmount = SPTokenAmount(token, _amount, decimals, currency);
      ...
      if (_tokenCount > 0)
        ...
        beneficiaryTokenCount = ISPController(directory.controllerOf(_projectId)).mintTokensOf(
          _projectId,
          _tokenCount,
          _beneficiary,
          '',
      ...
      // If a delegate was returned by the data source, issue a callback to it.
      if (_delegate != ISPPayDelegate(address(0))) {
        SPDidPayData memory _data = SPDidPayData(
          _payer,
          _projectId,
          _fundingCycle.configuration,
          _bundledAmount,
  ...
  */
  function _feeAmount(
    ...
    uint256 _discountedFee = _fee -
      PRBMath.mulDiv(_fee, _feeDiscount, SPConstants.MAX_FEE_DISCOUNT);

    // The amount of tokens from the `_amount` to pay as a fee.
    return
      _amount - PRBMath.mulDiv(_amount, SPConstants.MAX_FEE, _discountedFee + SPConstants.MAX_FEE);
  }
  ...
  */
  function _currentFeeDiscount(uint256 _projectId) internal view returns (uint256 feeDiscount) {
    ...
    if (directory.primaryTerminalOf(_PROTOCOL_PROJECT_ID, token) == ISPPaymentTerminal(address(0)))
      return SPConstants.MAX_FEE_DISCOUNT;

    // Get the fee discount.
    if (feeGauge == ISPFeeGauge(address(0)))
      feeDiscount = 0;
    ...
    if (feeDiscount > SPConstants.MAX_FEE_DISCOUNT) feeDiscount = 0;
  }
}
