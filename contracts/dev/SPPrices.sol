...
import './interfaces/ISPPrices.sol';
...
/** 
  ...
  @dev
  Adheres to -
  ISPPrices: General interface for the methods in this contract that interact with the blockchain's state according to the protocol's rules.

  @dev
  Inherits from -
  Ownable: Includes convenience functionality for checking a message sender's permissions before executing certain transactions.
*/
contract SPPrices is ISPPrices, Ownable {
  //*********************************************************************//
  // --------------------------- custom errors ------------------------- //
  //*********************************************************************//
  ...
  */
  mapping(uint256 => mapping(uint256 => ISPPriceFeed)) public override feedFor;
  ...
  */
  function priceFor(
    ...
    ISPPriceFeed _feed = feedFor[_currency][_base];

    // If it exists, return the price.
    if (_feed != ISPPriceFeed(address(0))) return _feed.currentPrice(_decimals);

    // Get the inverse feed.
    _feed = feedFor[_base][_currency];

    // If it exists, return the inverse price.
    if (_feed != ISPPriceFeed(address(0)))
  ...
  */
  function addFeedFor(
    ...
    if (feedFor[_currency][_base] != ISPPriceFeed(address(0))) revert PRICE_FEED_ALREADY_EXISTS();
