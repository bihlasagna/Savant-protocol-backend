...
import './interfaces/ISPReconfigurationBufferBallot.sol';
...
/** 
  ...
  @dev
  Adheres to -
  ISPReconfigurationBufferBallot: General interface for the methods in this contract that interact with the blockchain's state according to the protocol's rules.

  @dev
  Inherits from -
  ERC165: Introspection on interface adherance. 
*/
contract SPReconfigurationBufferBallot is ISPReconfigurationBufferBallot, ERC165 {
  //*********************************************************************//
  ...
  */
  mapping(uint256 => mapping(uint256 => SPBallotState)) public override finalState;
  ...
  */
  function stateOf(
    ...
    uint256 _configured,
    uint256 _start
  ) public view override returns (SPBallotState) {
    // If there is a finalized state, return it.
    if (finalState[_projectId][_configured] != SPBallotState.Active)
    ...
    if (block.timestamp < _configured + duration)
      ...
      return (block.timestamp >= _start) ? SPBallotState.Failed : SPBallotState.Active;
  ...
  */
  ...
  {
    return
      _interfaceId == type(ISPReconfigurationBufferBallot).interfaceId ||
      _interfaceId == type(ISPFundingCycleBallot).interfaceId ||
  ...
  */
  constructor(uint256 _duration, ISPFundingCycleStore _fundingCycleStore) {
    duration = _duration;
