...
import './interfaces/ISPSingleTokenPaymentTerminalStore.sol';
...
/**
  ...
  @dev
  Adheres to:
  ISPSingleTokenPaymentTerminalStore: General interface for the methods in this contract that interact with the blockchain's state according to the protocol's rules.

  @dev
  Inherits from -
  ReentrancyGuard: Contract module that helps prevent reentrant calls to a function.
*/
contract SPSingleTokenPaymentTerminalStore is ISPSingleTokenPaymentTerminalStore, ReentrancyGuard {
  ...
  */
  ISPFundingCycleStore public immutable override fundingCycleStore;
  ...
  */
  mapping(ISPSingleTokenPaymentTerminal => mapping(uint256 => uint256)) public override balanceOf;
  ...
  */
  mapping(ISPSingleTokenPaymentTerminal => mapping(uint256 => mapping(uint256 => uint256)))
    public
  ...
  */
  function currentReclaimableOverflowOf(
    ...
    SPFundingCycle memory _fundingCycle = fundingCycleStore.currentOf(_projectId);
    ...
    uint256 _totalSupply = ISPController(directory.controllerOf(_projectId))
  ...
  */
  constructor(
    ...
    ISPFundingCycleStore _fundingCycleStore,
    ISPPrices _prices
  ) {
  ...
  /**
    ...
    @dev
    The msg.sender must be an ISPSingleTokenPaymentTerminal. The amount specified in the params is in terms of the msg.sender's tokens.
  ...
  */
  function recordPaymentFrom(
    address _payer,
    SPTokenAmount calldata _amount,
    uint256 _projectId,
    ...
    )
  {
    ...
    if (fundingCycle.useDataSourceForPay()) {
      ...
      SPPayParamsData memory _data = SPPayParamsData(
        ISPSingleTokenPaymentTerminal(msg.sender),
        _payer,
        _amount,
        _projectId,
      ...
      (_weight, memo, delegate) = ISPFundingCycleDataSource(fundingCycle.dataSource()).payParams(
        _data
      );
    }
    ...
    balanceOf[ISPSingleTokenPaymentTerminal(msg.sender)][_projectId] =
      balanceOf[ISPSingleTokenPaymentTerminal(msg.sender)][_projectId] +
      _amount.value;
  ...
  /**
    ...
    @dev
    The msg.sender must be an ISPSingleTokenPaymentTerminal. The amount specified in the params is in terms of the msg.senders tokens.
  ...
  */
    ...
    )
  {
    ...
    // Scoped section prevents stack too deep. `_reclaimedTokenAmount`, `_currentOverflow`, and `_totalSupply` only used within scope.
    {
      ...
      SPTokenAmount memory _reclaimedTokenAmount;
      ...
      // Another scoped section prevents stack too deep. `_token`, `_decimals`, and `_currency` only used within scope.
      {
        ...
        address _token = ISPSingleTokenPaymentTerminal(msg.sender).token();

        // Get a reference to the terminal's decimals.
        uint256 _decimals = ISPSingleTokenPaymentTerminal(msg.sender).decimals();

        // Get areference to the terminal's currency.
        uint256 _currency = ISPSingleTokenPaymentTerminal(msg.sender).currency();
        ...
        // Get the number of outstanding tokens the project has.
        _totalSupply = ISPController(directory.controllerOf(_projectId)).totalOutstandingTokensOf(
          _projectId,
          fundingCycle.reservedRate()
        );
        ...
        _reclaimedTokenAmount = SPTokenAmount(_token, reclaimAmount, _decimals, _currency);
      }

      // If the funding cycle has configured a data source, use it to derive a claim amount and memo.
      if (fundingCycle.useDataSourceForRedeem()) {
        // Create the params that'll be sent to the data source.
        SPRedeemParamsData memory _data = SPRedeemParamsData(
          ISPSingleTokenPaymentTerminal(msg.sender),
          _holder,
          _projectId,
          fundingCycle.configuration,
        ...
        (reclaimAmount, memo, delegate) = ISPFundingCycleDataSource(fundingCycle.dataSource())
          .redeemParams(_data);
      } else {
        memo = _memo;
      }
    ...
    if (reclaimAmount > balanceOf[ISPSingleTokenPaymentTerminal(msg.sender)][_projectId])
  ...
  /**
    ...
    @dev
    The msg.sender must be an ISPSingleTokenPaymentTerminal. 
  ...
  */
  function recordDistributionFor(
    ...
    override
    nonReentrant
    returns (SPFundingCycle memory fundingCycle, uint256 distributedAmount)
  {
    // Get a reference to the project's current funding cycle.
    fundingCycle = fundingCycleStore.currentOf(_projectId);
    ...
    (uint256 _distributionLimitOf, uint256 _distributionLimitCurrencyOf) = ISPController(
      directory.controllerOf(_projectId)
    ).distributionLimitOf(
        _projectId,
        fundingCycle.configuration,
        ISPSingleTokenPaymentTerminal(msg.sender),
        ISPSingleTokenPaymentTerminal(msg.sender).token()
      );
    ...
    uint256 _balanceCurrency = ISPSingleTokenPaymentTerminal(msg.sender).currency();
    ...
    if (distributedAmount > balanceOf[ISPSingleTokenPaymentTerminal(msg.sender)][_projectId])
      revert INADEQUATE_PAYMENT_TERMINAL_STORE_BALANCE();

    // Store the new amount.
    usedDistributionLimitOf[ISPSingleTokenPaymentTerminal(msg.sender)][_projectId][
      fundingCycle.number
  ...
  */
  function recordUsedAllowanceOf(
    ...
    override
    nonReentrant
    returns (SPFundingCycle memory fundingCycle, uint256 usedAmount)
  {
    // Get a reference to the project's current funding cycle.
    fundingCycle = fundingCycleStore.currentOf(_projectId);
    ...
    (uint256 _overflowAllowanceOf, uint256 _overflowAllowanceCurrency) = ISPController(
      directory.controllerOf(_projectId)
    ).overflowAllowanceOf(
        _projectId,
        fundingCycle.configuration,
    ...
    usedOverflowAllowanceOf[ISPSingleTokenPaymentTerminal(msg.sender)][_projectId][
      fundingCycle.configuration
  ...
  */
  ...
  /**
    ...
    @dev
    The msg.sender must be an ISPSingleTokenPaymentTerminal. The amount returned is in terms of the msg.senders tokens.
  ...
  */
    ...
    returns (uint256 balance)
  {
    ...
    balance = balanceOf[ISPSingleTokenPaymentTerminal(msg.sender)][_projectId];

    // Set the balance to 0.
    balanceOf[ISPSingleTokenPaymentTerminal(msg.sender)][_projectId] = 0;
  }
  ...
  */
  function _reclaimableOverflowDuring(
    ...
    if (_redemptionRate == SPConstants.MAX_REDEMPTION_RATE) return _base;
    ...
    return
      PRBMath.mulDiv(
        ...
        _redemptionRate +
          PRBMath.mulDiv(
            _tokenCount,
            SPConstants.MAX_REDEMPTION_RATE - _redemptionRate,
            _totalSupply
          ),
        SPConstants.MAX_REDEMPTION_RATE
      );
  }
  ...
  */
  function _overflowDuring(
    ...
    (uint256 _distributionLimit, uint256 _distributionLimitCurrency) = ISPController(
      directory.controllerOf(_projectId)
  ...
  */
  function _currentTotalOverflowOf(
    ...
    ISPPaymentTerminal[] memory _terminals = directory.terminalsOf(_projectId);
    ...
    uint256 _totalOverflow18Decimal = _currency == SPCurrencies.ETH
      ? _ethOverflow
      : PRBMath.mulDiv(_ethOverflow, 10**18, prices.priceFor(SPCurrencies.ETH, _currency, 18));

    // Adjust the decimals of the fixed point number if needed to match the target decimals.
    return
      (_decimals == 18)
        ? _totalOverflow18Decimal
        : SPFixedPointNumber.adjustDecimals(_totalOverflow18Decimal, 18, _decimals);
  }
}
