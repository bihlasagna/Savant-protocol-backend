...
*/
contract SPTokenStore is ISPTokenStore, SPControllerUtility, SPOperatable {
  //*********************************************************************//
  // --------------------------- custom errors ------------------------- //
  //*********************************************************************//
  ...
  */
  ...
  ) SPOperatable(_operatorStore) SPControllerUtility(_directory) {
    projects = _projects;
  }
  ...
  */
  ...
  ) external override onlyController(_projectId) returns (ISPToken token) {
  ...
  */
  function changeFor(
    ...
    ISPToken _token,
    address _newOwner
  ) external override onlyController(_projectId) returns (ISPToken oldToken) {
  ...
  */
  function claimFor(
    ...
    uint256 _projectId,
    uint256 _amount
  ) external override requirePermission(_holder, _projectId, SPOperations.CLAIM) {
    // Get a reference to the project's current token.
    ISPToken _token = tokenOf[_projectId];
  ...
  */
  function transferFrom(
    ...
    address _recipient,
    uint256 _amount
  ) external override requirePermission(_holder, _projectId, SPOperations.TRANSFER) {
  ...
  */
  function shouldRequireClaimingFor(uint256 _projectId, bool _flag)
    external
    override
    requirePermission(projects.ownerOf(_projectId), _projectId, SPOperations.REQUIRE_CLAIM)
  {
    // Get a reference to the project's current token.
    ISPToken _token = tokenOf[_projectId];
