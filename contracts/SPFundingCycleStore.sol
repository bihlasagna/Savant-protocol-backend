...
import './abstract/SPControllerUtility.sol';
...
/** 
  ...
  @dev
  Adheres to -
  ISPTokenStore: General interface for the methods in this contract that interact with the blockchain's state according to the protocol's rules.

  @dev
  Inherits from -
  SPControllerUtility: Includes convenience functionality for checking if the message sender is the current controller of the project whose data is being manipulated.
*/
contract SPFundingCycleStore is ISPFundingCycleStore, SPControllerUtility {
  //*********************************************************************//
  // --------------------------- custom errors ------------------------- //
  //*********************************************************************//
  ...
  */
  function get(uint256 _projectId, uint256 _configuration)
    ...
    view
    override
    returns (SPFundingCycle memory fundingCycle)
  {
  ...
  */
  function currentBallotStateOf(uint256 _projectId) external view override returns (SPBallotState) {
    // Get a reference to the latest funding cycle configuration.
    uint256 _fundingCycleConfiguration = latestConfigurationOf[_projectId];

    // Resolve the funding cycle for the latest configuration.
    SPFundingCycle memory _fundingCycle = _getStructFor(_projectId, _fundingCycleConfiguration);
  ...
  */
  ...
  constructor(ISPDirectory _directory) SPControllerUtility(_directory) {}
  ...
  */
  function configureFor(
  ...
  ) external override onlyController(_projectId) returns (SPFundingCycle memory) {
    ...
    if (_data.discountRate > SPConstants.MAX_DISCOUNT_RATE) revert INVALID_DISCOUNT_RATE();
    ...
    if(_data.ballot != ISPFundingCycleBallot(address(0))) {
      ...
      try _data.ballot.supportsInterface(type(ISPFundingCycleBallot).interfaceId) returns (bool _supports) {
    ...
    if (
      _data.ballot != ISPFundingCycleBallot(address(0)) ||
      _data.duration > 0 ||
  ...
  */
  function _configureIntrinsicPropertiesFor(
    ...
    SPFundingCycle memory _baseFundingCycle = _getStructFor(_projectId, _currentConfiguration);
    ...
    uint256 _timestampAfterBallot = _baseFundingCycle.ballot == ISPFundingCycleBallot(address(0))
      ? 0
  ...
  */
  function _initFor(
    uint256 _projectId,
    SPFundingCycle memory _baseFundingCycle,
    uint256 _configuration,
    uint256 _mustStartAtOrAfter,
    uint256 _weight
  ) private {
  ...
  */
  function _standbyOf(uint256 _projectId) private view returns (uint256 configuration) {
    ...
    SPFundingCycle memory _fundingCycle = _getStructFor(_projectId, configuration);
    ...
    SPFundingCycle memory _baseFundingCycle = _getStructFor(_projectId, _fundingCycle.basedOn);
  ...
  */
  function _mockFundingCycleBasedOn(SPFundingCycle memory _baseFundingCycle, bool _allowMidCycle)
    private
    view
  ...
  */
  function _deriveStartFrom(SPFundingCycle memory _baseFundingCycle, uint256 _mustStartAtOrAfter)
    private
    pure
  ...
  */
  function _deriveWeightFrom(SPFundingCycle memory _baseFundingCycle, uint256 _start)
    private
    pure
    ...
    if (_baseFundingCycle.duration == 0)
      return
        PRBMath.mulDiv(
          ...
          SPConstants.MAX_DISCOUNT_RATE - _baseFundingCycle.discountRate,
          SPConstants.MAX_DISCOUNT_RATE
        );
  ...
  */
  function _deriveNumberFrom(SPFundingCycle memory _baseFundingCycle, uint256 _start)
    private
    pure
    returns (uint256)
  {
  ...
  */
  function _isApproved(uint256 _projectId, SPFundingCycle memory _fundingCycle)
    private
    view
    returns (bool)
  {
  ...
  */
  function _ballotStateOf(
  ...
  ) private view returns (SPBallotState) {
    // If there is no ballot funding cycle, implicitly approve.
    if (_ballotFundingCycleConfiguration == 0) return SPBallotState.Approved;

    // Get the ballot funding cycle.
    SPFundingCycle memory _ballotFundingCycle = _getStructFor(
      _projectId,
    ...
    if (_ballotFundingCycle.ballot == ISPFundingCycleBallot(address(0)))
      return SPBallotState.Approved;
    else if (_ballotFundingCycle.ballot.duration() >= block.timestamp - _configuration)
      return SPBallotState.Active;
  ...
  */
    ...
    returns (SPFundingCycle memory fundingCycle)
  {
    ...
    fundingCycle.ballot = ISPFundingCycleBallot(address(uint160(_packedUserProperties)));
