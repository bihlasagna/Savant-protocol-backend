...
import './abstract/SPOperatable.sol';
import './interfaces/ISPController.sol';
import './interfaces/ISPMigratable.sol';
import './interfaces/ISPOperatorStore.sol';
...
*/
contract SPController is ISPController, ISPMigratable, SPOperatable, ERC165 {
  ...
  */
  mapping(uint256 => mapping(uint256 => mapping(ISPPaymentTerminal => mapping(address => uint256))))
  ...
  */
  function launchProjectFor(
    ...
    SPGroupedSplits[] calldata _groupedSplits,
    SPFundAccessConstraints[] calldata _fundAccessConstraints,
  ...
  */
  function launchFundingCyclesFor(
    ...
    virtual
    override
    requirePermission(projects.ownerOf(_projectId), _projectId, SPOperations.RECONFIGURE)
  ...
  */
  function issueTokenFor(
    ...
    virtual
    override
    requirePermission(projects.ownerOf(_projectId), _projectId, SPOperations.ISSUE)
  ...
  */
  function changeTokenOf(
    ...
    virtual
    override
    requirePermission(projects.ownerOf(_projectId), _projectId, SPOperations.CHANGE_TOKEN)
  {
  ...
  function mintTokensOf(
    ...
    // Scoped section prevents stack too deep. `_fundingCycle` only used within scope.
    {
      ...
      _requirePermissionAllowingOverride(
        ...
        _projectId,
        SPOperations.MINT,
        directory.isTerminalOf(_projectId, ISPPaymentTerminal(msg.sender)) ||
    ...
    if (_reservedRate == SPConstants.MAX_RESERVED_RATE)
      ...
      beneficiaryTokenCount = PRBMath.mulDiv(
        _tokenCount,
        SPConstants.MAX_RESERVED_RATE - _reservedRate,
        SPConstants.MAX_RESERVED_RATE
      );
  ...
  */
  function migrate(uint256 _projectId, ISPMigratable _to)
    external
    virtual
    override
    requirePermission(projects.ownerOf(_projectId), _projectId, SPOperations.MIGRATE_CONTROLLER)
  {
    ...
    returns (uint256 tokenCount)
  {
    ...
    uint256 _leftoverTokenCount = tokenCount == 0
      ...
      : _distributeToReservedTokenSplitsOf(
        ...
        SPSplitsGroups.RESERVED_TOKENS,
        tokenCount
      );
    ...
    for (uint256 _i = 0; _i < _splits.length; _i++) {
      ...
      if (_tokenCount > 0) {
        tokenStore.mintFor(
          ...
          _split.allocator != ISPSplitAllocator(address(0))
  ...
  */
  function _configure(
    ...
    if (_metadata.reservedRate > SPConstants.MAX_RESERVED_RATE) revert INVALID_RESERVED_RATE();

    // Make sure the provided redemption rate is valid.
    if (_metadata.redemptionRate > SPConstants.MAX_REDEMPTION_RATE)
      revert INVALID_REDEMPTION_RATE();

    // Make sure the provided ballot redemption rate is valid.
    if (_metadata.ballotRedemptionRate > SPConstants.MAX_REDEMPTION_RATE)
      revert INVALID_BALLOT_REDEMPTION_RATE();

    // Configure the funding cycle's properties.
    SPFundingCycle memory _fundingCycle = fundingCycleStore.configureFor(
      _projectId,
      _data,
      SPFundingCycleMetadataResolver.packFundingCycleMetadata(_metadata),
      _mustStartAtOrAfter
    );
    ...
    for (uint256 _i; _i < _fundAccessConstraints.length; _i++) {
      SPFundAccessConstraints memory _constraints = _fundAccessConstraints[_i];
  ...
  */
  function _reservedTokenAmountFrom(
    ...
    if (_reservedRate == SPConstants.MAX_RESERVED_RATE) return _unprocessedTokenBalanceOf;
