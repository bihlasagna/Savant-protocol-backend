...
import './interfaces/ISPDirectory.sol';
...
/**
  ...
  @dev
  Adheres to -
  ISPSplitsStore: General interface for the methods in this contract that interact with the blockchain's state according to the protocol's rules.
...
*/
contract SPSplitsStore is ISPSplitsStore, SPOperatable {
  //*********************************************************************//
  // --------------------------- custom errors ------------------------- //
  //*********************************************************************//
...
*/
  function splitsOf(
    ...
    uint256 _domain,
    uint256 _group
  ) external view override returns (SPSplit[] memory) {
  ...
  */
    ...
    )
  {
    ...
    for (uint256 _i = 0; _i < _groupedSplitsLength; ) {
      ...
      SPGroupedSplits memory _groupedSplit = _groupedSplits[_i];
  ...
  */
  function _set(
    ...
    uint256 _domain,
    uint256 _group,
    SPSplit[] memory _splits
  ) internal {
    // Get a reference to the project's current splits.
    SPSplit[] memory _currentSplits = _getStructsFor(_projectId, _domain, _group);
    ...
    for (uint256 _i = 0; _i < _splits.length; _i++) {
      ...
      if (_percentTotal > SPConstants.SPLITS_TOTAL_PERCENT) revert INVALID_TOTAL_PERCENT();
      ...
      if (_splits[_i].lockedUntil > 0 || _splits[_i].allocator != ISPSplitAllocator(address(0))) {
  ...
  */
  function _getStructsFor(
    ...
    uint256 _domain,
    uint256 _group
  ) private view returns (SPSplit[] memory) {
    ...
    SPSplit[] memory _splits = new SPSplit[](_splitCount);
    ...
    for (uint256 _i = 0; _i < _splitCount; _i++) {
      ...
      if (_packedSplitPart2 > 0) {
        ...
        // allocator in bits 48-207.
        _split.allocator = ISPSplitAllocator(address(uint160(_packedSplitPart2 >> 48)));
      }
